def place_order(menu):
    ...
    for index, item in menu_items.items():
        name = item["Item name"]
        price = item["Price"]
        category, meal = name.split(" - ", 1)
        print_menu_line(index, category, meal, price)

    while True:
        selection = input("Type menu number: ").strip()

        if selection.lower() == 'n':
            print("Thank you for your order.")
            break
        else:
            order = update_order(order, selection, menu_items)
...
def update_order(order, menu_selection, menu_items):
    if menu_selection.isdigit():
        ...
        if item_number in menu_items:
            ...
            print("Would you like to keep ordering? (N) to quit:", end="")
...
def print_itemized_receipt(receipt):
    ...
    for item in receipt:
        ...
        print_receipt_line(item["Item name"], price, quantity)
